...
def check_camera_motion( lastFrame,frame,theta_thes,trans_thes):
    # add keyframe

    prev_rot = lastFrame[:3,:3]
    prev_trans = lastFrame[:3,3]
    curr_rot = frame[:3,:3]
    curr_trans = frame[:3,3]
    _, theta_diff = rot_compare(prev_rot, curr_rot)
    _, l2_diff = trans_compare(prev_trans, curr_trans)

    if theta_diff < theta_thes or l2_diff < trans_thes:
        return True
    else:
        return False