...
def remove_points(to_remove, params, params_opt_exclude, variables, optimizer):
    ...
    variables["influence"] = variables["influence"][to_keep]
    variables["infl_denom"] = variables["infl_denom"][to_keep]
...
def add_influence_stats(variables, influence):
    variables["influence"] += influence.detach()
    variables["infl_denom"][influence>0] += 1

    return variables

def prune_influence(params, params_opt_exclude, variables, optimizer, quantile_threshold):
    threshold = torch.quantile(variables["influence"],quantile_threshold)
    prune_mask = variables["influence"]<=threshold
    params, variables = remove_points(prune_mask, params, params_opt_exclude, variables, optimizer)
    variables["influence"] *= 0
    variables["infl_denom"] *= 0
    return params, variables
...
def influ_densify(params, variables, optimizer, densify_dict, params_opt_exclude,quantile_threshold, device="cuda"):

    variables = accumulate_mean2d_gradient(variables)
    grad_thresh = densify_dict['grad_thresh']
    influence_threshold = torch.quantile(variables["influence"],quantile_threshold)
    influ_mask = variables["influence"]>=influence_threshold

    grads = variables['means2D_gradient_accum'] / variables['denom']
    grads[grads.isnan()] = 0.0
    to_clone =influ_mask & torch.logical_and(grads >= grad_thresh, (
            torch.max(torch.exp(params['log_scales']), dim=1).values <= 0.01 * variables['scene_radius']))
    new_params = {k: v[to_clone] for k, v in params.items() if k not in ['cam_unnorm_rots', 'cam_trans']}
    params = cat_params_to_optimizer(new_params, params, params_opt_exclude, optimizer)
    num_pts = params['means3D'].shape[0]
    ...
    new_timesteps = variables['timestep'][to_clone]
    new_influence = variables['influence'][to_clone]
    new_influence_denom = variables['infl_denom'][to_clone]
    variables['timestep'] = torch.cat([variables['timestep'], new_timesteps])
    variables["influence"] = torch.cat([variables['influence'], new_influence])
    variables['infl_denom'] = torch.cat([variables['infl_denom'],new_influence_denom])

    influence_threshold = torch.quantile(variables["influence"],quantile_threshold)
    influ_mask = variables["influence"]>=influence_threshold

    to_split =influ_mask & torch.logical_and(padded_grad >= grad_thresh,
                                                torch.max(torch.exp(params['log_scales']), dim=1).values > 0.01 * variables[
                                                    'scene_radius'])
    n = densify_dict['num_to_split_into']  # number to split into

    parent_timesteps = variables['timestep'][to_split]
    parent_influence = variables['influence'][to_split]
    parent_infl_denom = variables['infl_denom'][to_split]
    new_timesteps = parent_timesteps.repeat(n)
    variables['timestep'] = torch.cat([variables['timestep'], new_timesteps])
    variables['influence'] = torch.cat([variables['influence'], parent_influence.repeat(n)])
    variables['infl_denom'] = torch.cat([variables['infl_denom'], parent_infl_denom.repeat(n)])
    ...
    for k, v in params.items():
        ...
        selected = v[to_split]
        if selected.dim() == 1:
            new_params[k] = selected.repeat(n)        # 1D: [K] -> [n*K]
        elif selected.dim() == 2:
            new_params[k] = selected.repeat(n, 1)     # 2D: [K, D] -> [n*K, D]
    stds = torch.exp(params['log_scales'])[to_split].repeat(n, 3)
    means = torch.zeros((stds.size(0), 3), device=device)
    samples = torch.normal(mean=means, std=stds)
    rots = build_rotation(params['unnorm_rotations'][to_split], device=device).repeat(n, 1, 1)
...
def split_influence(params, variables, optimizer, densify_dict, params_opt_exclude, quantile_threshold, device="cuda"):

    influence_threshold = torch.quantile(variables["influence"],quantile_threshold)
    to_split = variables["influence"]>=influence_threshold
    variables = accumulate_mean2d_gradient(variables)
    grad_thresh = densify_dict['grad_thresh']
    grads = variables['means2D_gradient_accum'] / variables['denom']
    grads[grads.isnan()] = 0.0
    ...
    padded_grad = torch.zeros(num_pts, device=device)
    padded_grad[:grads.shape[0]] = grads
    to_split = to_split & torch.logical_and(padded_grad >= grad_thresh,
                                 torch.max(torch.exp(params['log_scales']), dim=1).values > 0.01 * variables[
                                     'scene_radius'])
    print(f"Split by influence: {to_split.sum().item()} points")
    n = densify_dict['num_to_split_into']  # number to split into
    #new_params = {k: v[to_split].repeat(n, 1) for k, v in params.items() if k not in ['cam_unnorm_rots', 'cam_trans']}
    new_params = {}
    # ✅ 正确处理 1D 和 2D 参数
    ...
    variables['denom'] = torch.zeros(num_pts, device=device)
    variables['max_2D_radius'] = torch.zeros(num_pts, device=device)
    variables['influence'] = torch.zeros(num_pts, device=device)
    variables['infl_denom'] = torch.zeros(num_pts, device=device)
