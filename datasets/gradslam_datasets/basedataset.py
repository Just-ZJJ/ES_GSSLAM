...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # self.color_paths, self.depth_paths, self.semantic_id_paths, self.semantic_color_paths, self.embedding_paths, self.gt_semantic_id_paths, self.gt_semantic_color_paths, = self.get_filepaths()
        ...
        if self.load_semantics:
            ...
            # self.gt_semantic_id_paths = self.gt_semantic_id_paths[self.start : self.end : stride]
            # self.gt_semantic_color_paths = self.gt_semantic_color_paths[self.start : self.end : stride]
        if self.load_embeddings:
    ...
    def __getitem__(self, index):
        ...
        if self.load_semantics:
            ...
            # gt_semantic_id_path = self.gt_semantic_id_paths[index]
            # gt_semantic_id = np.asarray(imageio.imread(gt_semantic_id_path), dtype=np.int64)
            # gt_semantic_id = self._preprocess_semantic_id(gt_semantic_id)
            # gt_semantic_id = torch.from_numpy(gt_semantic_id)
            ...
            # gt_semantic_color_path = self.gt_semantic_color_paths[index]
            # gt_semantic_color = np.asarray(imageio.imread(gt_semantic_color_path), dtype=float)
            # gt_semantic_color = self._preprocess_semantic_color(gt_semantic_color)
            # gt_semantic_color = torch.from_numpy(gt_semantic_color)
        ...
        if self.load_semantics:
            ...
            # return_data = return_data + (gt_semantic_id.to(self.device),
                                        #  gt_semantic_color.to(self.device).type(self.dtype)) # semantic_id has int dtype.
        if self.load_embeddings:
