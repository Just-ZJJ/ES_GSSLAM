...
def get_mono_depth_estimator(cfg):
    ...
    depth_model = cfg["driod_tracking"]["mono_prior"]["depth"]
    depth_pretrained = cfg["driod_tracking"]["mono_prior"]["depth_pretrained"]
...
def get_omnidata_model(pretrained_path, device, num_channels):
    from submodules.mono_priors.omnidata.modules.midas.dpt_depth import DPTDepthModel
...
def predict_mono_depth(model,idx,input,cfg,device):
    ...
    '''
    depth_model = cfg["driod_tracking"]["mono_prior"]["depth"]
    output_dir = f"{cfg['workdir']}/{cfg['run_name']}"
    if depth_model == "omnidata":
        # s = cfg["cam"]["H_out"]
        # image_size = (s,s)
        image_size = (512,512)
    ...
    os.makedirs(os.path.dirname(output_path_np), exist_ok=True)
